...
sap.ui.define(["sap/ui/base/Object"], function(Object) {
	...
	return Object.extend("nw.epm.refapps.st.overview.localService.MockRequests", {
		...
		getRequests: function() {
			...
			return [this.mockApprovePo(), this.mockRejectPo(), this.mockDeleteProduct()];
		},
		...
		mockApprovePo: function() {
			return {
				...
				method: "POST",
				path: new RegExp("SEPMRA_C_OVW_POApproveApprove\\?PurchaseOrder='(.*)'"),
		...
		mockRejectPo: function() {
			return {
				...
				method: "POST",
				path: new RegExp("SEPMRA_C_OVW_POApproveReject\\?PurchaseOrder='(.*)'"),
				response: function(oXhr, sPOId) {
					this.deletePo(oXhr, sPOId);
				}.bind(this)
			};
		...
		mockDeleteProduct: function() {
			return {
				// This mock request simulates the function import "Delete_ext",
				// which is triggered when the user chooses the "Delete" button.
				// It removes the product from the mock data.
				method: "POST",
				path: new RegExp("SEPMRA_C_OVW_NoSellDelete_ext\\?Product='(.*)'"),
				response: function(oXhr, sProdId) {
					var aProducts = this._oMockServer.getEntitySetData("SEPMRA_C_OVW_NoSell"),
						filterProducts = function(oProduct) {
							return oProduct.Product !== sProdId;
						};
					aProducts = aProducts.filter(filterProducts);
					this._oMockServer.setEntitySetData("SEPMRA_C_OVW_NoSell", aProducts);
					oXhr.respondJSON(200, {}, JSON.stringify({
						d: {
							results: []
						}
